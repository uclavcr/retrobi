...
/**
 * A link that cleans all removable nodes from the attribute tree of the given
 * card after click (makes the attribute tree more compact).
 * 
 * @author Vojtěch Hordějčuk
 */
public class CleanTreeLink extends Link<Object> {
    /**
     ...
     * card ID
     */
    public CleanTreeLink(final String id, final String cardId) {
        super(id);
    ...
    public void onClick() {
        try {
            ...
            final AttributeNode root = SimpleAttributeUtils.fromDocumentEnsured(card, prototype);
            
            // clear the removable nodes
            
            if (SimpleAttributeUtils.removeEmptyChildren(root)) {
                this.info("V položkovém rozpisu byly nalezeny nepotřebné uzly.");
            } else {
                this.info("V položkovém rozpisu nebyly nalezeny žádné nepotřebné uzly.");
            }
            ...
            this.info("Položkový rozpis byl vyčištěn a uložen k lístku.");
